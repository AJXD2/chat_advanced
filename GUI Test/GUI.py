...
class MyGui(QMainWindow):
    def __init__(self):
        ...
        self.ServerListBox.addItem(QtGui.QIcon('error.png'), 'Connecting...')
        self.watch_key = threading.Thread(target=self.keyloop)
        self.watch_key.start()
        threading.Thread(target=self.scan_server_list).start()
    ...
    def scan_server_list(self):
        ...
        try:
            ...
            return
        self.first = True
        servers = self.serverlist['servers']
        icons = self.serverlist['icons']
        if len(icons) != len(servers):
            error = QErrorMessage(self)
            error.setWindowTitle('Serverlist Error')
            error.setWindowIcon(QtGui.QIcon('error.png'))
            error.showMessage('The server send a malformed request. Please contact AJXD2#6477 on discord now.')
            return
        self.ServerListBox.clear()
        for i in range(len(servers)):
            icon = icons[i]
            self.ServerListBox.addItem(servers[i])
            
            #self.ServerListBox.setItemIcon(i ,self.iconFromBase64(base64=icon))
            print(i)
        #self.ServerListBox.clear()
    
    def iconFromBase64(self, base64_in: str):
        pixmap = QtGui.QPixmap()
        pixmap.loadFromData(base64.b64decode(base64_in))
        icon = QIcon(pixmap)
        return icon
    def connect(self):
        while True:

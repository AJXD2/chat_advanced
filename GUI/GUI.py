...
class MyGui(QMainWindow):
    def __init__(self):
        ...
        self.watch_key = threading.Thread(target=self.keyloop, daemon=True)
    ...
    def login(self):
        try:
            ...
            if self.PASSWORD == None or '':
                return
            request = requests.get(f'{self.api_url}auth/{self.api_token}', json={'user': self.USERNAME, 'password': hash(self.PASSWORD)})
